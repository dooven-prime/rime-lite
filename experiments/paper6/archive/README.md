# Paper VI Archive

These scripts are historical exploratory support, not active Paper VI theorem
entry points.

Active scripts live one directory up:

- `tangent_commutator_map.py`
- `fragmentation_walls.py`
- `generator_moduli_space.py`
- `wall_crossing_summary.py`
- `phase_utils.py`

Archived here:

- `bifurcation_analysis.py` - superseded by `generator_moduli_space.py`, which
  now also writes `_paper6_bifurcation_log.txt`.
- `commutativity_wall_geometry.py` - superseded by `tangent_commutator_map.py`
  and retained as earlier local-scan evidence.
- `deformation_wall_crossing.py` - verbose Type III/IV demonstrations,
  condensed by `wall_crossing_summary.py`. Type III/IV are Paper V/VII
  mechanism examples, not Paper VI wall categories.

Moved out:

- `search_type4_*.py` moved to
  `experiments/paper7/archive/represented_incidence_search/`, because those
  scripts explore whether Type IV incidence appears in represented systems,
  which is now part of the Paper VII completion/incidence boundary line.
