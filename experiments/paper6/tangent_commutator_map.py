...
Status: numerical computation of T_1 Sigma_comm 鈥?the tangent space of the
commutativity locus at the full-generator point w = (1,...,1).
...
Derivation
----------
At w = 1, Q_T(1) = QT_all and H_T(1) = HT_all commute ([QT_all, HT_all] = 0).
...
At w = 1 (using [QT_all, HT_all] = 0):
    For k in QT:  J_k = [rho(g_k), HT_all] / 12
    For k in HT:  J_k = [QT_all, rho(g_k)] / 6

The tangent space T_1 Sigma_comm is the kernel of the linear map
    delta_w |-> sum_k delta_w_k * J_k.

This script:
1. Computes all 18 Jacobian matrices J_k.
2. Flattens them to real vectors and computes SVD.
3. Reports the kernel basis 鈥?which weight directions stay in Sigma_comm
   to first order.
...
def jacobian_matrices(
    ...
    for k in range(len(keys)):
        if k in qt_idx:
            Jk = (rhos[k] @ HT - HT @ rhos[k]) / len(qt_idx)
            kind = "QT"
        else:
            Jk = (QT @ rhos[k] - rhos[k] @ QT) / len(ht_idx)
            kind = "HT"
        J.append(Jk)
...
def flatten_hermitian(J_list: list[np.ndarray]) -> np.ndarray:
    """Flatten each skew-Hermitian J_k to a real vector.

    A skew-Hermitian matrix A = -A^H has:
    - real diagonal (which is zero)
    - complex upper triangle with a_ji = -conj(a_ij)

    We encode each J_k as:
    - real parts of upper-triangular entries (i < j)
    - imag parts of upper-triangular entries (i < j)
    Total real DoF per NxN skew-Hermitian: N(N-1).

    Returns (N*(N-1), m) real matrix.
    """
    N = J_list[0].shape[0]
    m = len(J_list)

    rows, cols = np.triu_indices(N, k=1)
    n_dof = len(rows)  # N*(N-1)/2 complex entries = N*(N-1) real DoF

    flat = np.zeros((2 * n_dof, m), dtype=float)
    for k, Jk in enumerate(J_list):
        upper = Jk[rows, cols]  # complex entries, length n_dof
        flat[:n_dof, k] = upper.real
        flat[n_dof:, k] = upper.imag
    return flat
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compute the Paper VI tangent commutator map snapshot."
    )
    parser.add_argument(
        "--deep",
        action="store_true",
...
def main() -> None:
    ...
    rows, cols = np.triu_indices(N, k=1)
    ndof = len(rows)
    flat_J = flatten_hermitian(J_list)
    ...
    print(f"Nullity (dim T_1 Sigma_comm): {svd['nullity']}")
    ...
    print("Hand-picked directions projected onto T_1 Sigma_comm:")
    ...
    print()
    print(f"Directions in T_1 Sigma_comm: {inside}")
    print(f"Directions leaving T_1 Sigma_comm: {outside}")
    print()
    ...
    # Total DoF in skew-Hermitian 228x228: 228*227 = 51756
    # Flattened Jacobian dimension: 51756 x 18
    # We expect the Jacobian to have full column rank minus kernel dim

    print("Geometric summary:")
    print(f"  Total weight space dim: {m}")
    print(f"  dim T_1 Sigma_comm (kernel): {svd['nullity']}")
    print(f"  Codimension of Sigma_comm at canonical point: {svd['rank']}")
    print(f"  Singular value spectrum suggests:")
    if svd["rank"] == m:
        print(f"    Sigma_comm is a smooth point of codimension {m}")
    else:
        print(f"    Sigma_comm has tangent space of dimension {svd['nullity']}")
    print()
    ...
    print(f"Expected inside T_1 Sigma_comm: {sorted(expected_inside)}")
    print(f"Found inside T_1 Sigma_comm: {sorted(found_inside)}")

    if found_inside == expected_inside:
        print("[OK] All expected symmetry directions confirmed in T_1 Sigma_comm.")
    else:
    ...
    if single_del_inside:
        ...
        print("[OK] No single-coordinate deletions are in T_1 Sigma_comm.")

    # Assertions for Theorem 1
    ...
    if args.deep:
        ...
        for trial in range(10):
            ...
            for eps in [0.01, 0.1, 0.5, 1.0]:
                ...
                flat_w = np.zeros((2 * ndof, m))
                for k, Jk in enumerate(Jw):
                    upper = Jk[rows, cols]
                    flat_w[:ndof, k] = upper.real
                    flat_w[ndof:, k] = upper.imag
    ...
    write_snapshot_log([
        "Paper VI tangent commutator map snapshot",
    ...
    print("\n[snapshot OK: T_1 Sigma_comm tangent space computed]")
