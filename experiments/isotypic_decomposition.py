...
Uses CubieSpectralOperator._full_commutant_combinatorial() for exact 228-dim
...
Usage:
    python test/canonical/_exp_isotypic_decomposition.py
"""
import numpy as np
...
def isotypic_decomposition(op):
    ...
    """
    full_basis, full_comm_dim = op._full_commutant_combinatorial()
...
def isotypic_transport_tensor(op):
    ...
    rho_mats = []
    for _k, (_mv, rho, *_details) in op.rho_moves.items():
        rho_mats.append(rho.toarray() if hasattr(rho, 'toarray') else np.array(rho))
    n_gens = len(rho_mats)
...
def analyze_isotypic():
    ...
    print("=" * 72)
    print(f"Full commutant dimension: 610")
    print(f"Sum of per-layer comm_dim: {iso_result['dim_total']}")
    print(f"Overcompleteness ratio: {iso_result['dim_total']}/610 = {iso_result['dim_total']/610:.2f}")
    print()
    ...
    print(f"  Full-space: 610")
    print(f"  Sum of per-layer: {iso_result['dim_total']}")
    print(f"  Ratio: {iso_result['dim_total']/610:.2f} (> 1 = overcomplete)")
