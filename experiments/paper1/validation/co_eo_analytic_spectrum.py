"""Symmetry-guided spectral checks for the registered CO and EO blocks.

CO block: qualified proposition (O dimension pattern + numerical spectrum)
  Verifies: diagonal=1/2, Tr=4, adjacency classes, O orbit regularity,
            accidental A₁/A₂ degeneracy, M-spectrum,
...
# CO block: mixed status (O dimension pattern plus numerical degeneracy).
# Complex-linear O covariance is audited separately by
# experiments/paper2/symmetry_and_transport_audit.py.
# EO block: Numerical-Representation Observation (2T₂ multiplicity blocks Schur closure)
status_co = {"passed": 0, "total": 0, "label": "MIXED"}
...
# ═══════════════════════════════════════════════════════════════
# 1. CO Block — Symmetry-Guided Computation
# ═══════════════════════════════════════════════════════════════

start_block("CO")
print("=" * 65)
print("1. CO BLOCK (8-dim) — Symmetry-Guided Spectrum")
print("=" * 65)
...
# --- 1.6 O orbit regularity ---
print("\n--- 1.6 O Orbit Regularity (necessary-pattern check) ---")
# This checks orbit-regular scalar data only; it is not a covariance proof.
check(np.allclose(np.diag(A_co), 0.5), "All 8 diagonals equal (O-transitive pattern)")
...
# --- 2.7 Registered family comparison ---
# Compare the registered 18-full result with two additional declared families.
# Different generator families define different averaging operators and spectra.
print("\n--- 2.7 Registered Family Comparison ---")
...
# Record the 18-full multiplicity pattern separately. Other families have
# different spectra and multiplicities because they define different operators.
...
print(f"    O dimension pattern + registered covariance audit + numerical degeneracy")
...
print(f"    λ = {{8/9×2, 7/9×3, 5/9×7}} — registered 18-full result")
print(f"    k-set = {{1,2,4}} for face-turn averaging (differs for other families)")
print(f"    (2T₂ multiplicity keeps this result at observation status)")
print(f"\n  Open / acknowledged gaps:")
print(f"    - cp block: Hamming k (Bose-Mesner internal) != global k (A eigenvalues).")
print(f"      These two parameters must remain distinct in manuscript interpretation.")
print(f"    - The canonical face partition does not satisfy the stated integer hypothesis.")
print(f"    - No algebraic classification or universal rationality claim is made.")
