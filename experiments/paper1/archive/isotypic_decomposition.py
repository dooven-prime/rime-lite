"""Compressed-commutant census and transport diagnostics (F1/F2/F3/F4).

Uses CubieSpectralOperator.full_commutant_combinatorial() for the 228-dimensional
commutant (610 basis matrices), compresses it to each spectral layer, and then
computes a numerical central decomposition and transport diagnostics. In the
current canonical run, F1 returns 51 compressed components. Because a spectral
layer need not be G-invariant, this computation is not by itself a theorem that
the components are isotypic subrepresentations of G.

Current F1 census:
- 51 compressed components and 59 reported summands
- full commutant dimension 610; sum of compressed per-layer dimensions 966
- typical F1 runtime is about 90 seconds on the current reference machine
...
Archived usage:
    python experiments/paper1/archive/isotypic_decomposition.py
"""
import numpy as np
...
def isotypic_decomposition(op):
    """Numerical central decomposition of the commutant compressed to each layer.

    The full commutant is computed combinatorially in the ambient space. The
    compressed layerwise objects require a separate invariance/closure argument
    before they can be interpreted as group-isotypic decompositions.
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--f1-only",
        action="store_true",
        help="run only the compressed-commutant census used by Paper I Appendix B",
    )
    ...
    if args.f1_only:
        ...
        iso_result = isotypic_decomposition(op)
        print(f"Full commutant dimension: {iso_result['full_comm_dim']}")
        print(f"Sum of compressed per-layer dimensions: {iso_result['dim_total']}")
        print(f"Compressed components: {iso_result['total_isotypic_types']}")
        for lam in sorted(iso_result['blocks'], reverse=True):
            block = iso_result['blocks'][lam]
            print(
                f"lambda={lam:.6f} dim={block['dim']} "
                f"comm={block['commutant_dim']} center={block['center_dim']} "
                f"components={block['isotypic']}"
            )
    else:
