...
Computationally verified:
  ...
  - Semisimple: generated finite-dimensional unital *-algebra
  - Center Z(A) = 8-dim (one scalar per simple component)
  - Killing-form degeneracy agrees numerically with the center dimension
  - Isotypic decomposition on EP(144): 8 components from center eigenspaces
  - The eight registered central components have dimensions 24 x 4 and 12 x 4
...
import numpy as np
import os, sys, time
from rime.algebra import audit_finite_dimensional_star_algebra
...
# The EP quarter-turn averages are real in this realization. Work over an
# explicit real array so the trace and structure-constant audits do not
# silently discard complex roundoff.
for name, matrix in (("Q0", Q0), ("Q1", Q1), ("Q2", Q2)):
    imag_residual = np.max(np.abs(matrix.imag))
    assert imag_residual < TOL, f"{name} imaginary residual: {imag_residual:.2e}"
Q0 = np.asarray(Q0.real, dtype=np.float64)
Q1 = np.asarray(Q1.real, dtype=np.float64)
Q2 = np.asarray(Q2.real, dtype=np.float64)
...
# 3. Semisimplicity via the finite-dimensional *-algebra theorem.
# The Frobenius Gram matrix of an SVD basis only checks linear independence;
# it is not a Jacobson-radical criterion.
generator_hermiticity = max(
    np.linalg.norm(Q - Q.conj().T, 'fro') for Q in (Q0, Q1, Q2)
)
algebra_certificate = audit_finite_dimensional_star_algebra(
    [Q0, Q1, Q2], svd_basis, tol=TOL
)
print(f"  Generator Hermiticity residual: {generator_hermiticity:.3e}")
print(
    "  Identity-in-algebra residual: "
    f"{algebra_certificate['identity_span_residual']:.3e}"
)
print(
    "  Multiplication-closure residual: "
    f"{algebra_certificate['maximum_product_closure_residual']:.3e}"
)
print(
    "  Algebra *-closure residual: "
    f"{algebra_certificate['maximum_adjoint_closure_residual']:.3e}"
)
print(
    "  SVD-basis orthonormality residual: "
    f"{algebra_certificate['declared_basis_gram_residual']:.3e}"
)
assert generator_hermiticity < TOL
assert algebra_certificate['registered_unital_star_algebra']
assert algebra_certificate['semisimplicity_supported']
print("  Numerically registered finite-dimensional unital *-algebra")
print("  Semisimplicity follows from the finite-dimensional C*-algebra theorem")
...
print(f"  Killing degeneracy dim = {k_zero}; center dimension = {center_dim} (consistency diagnostic)")
...
# Each 24-dimensional central component realizes one M₂ simple module with
# multiplicity 12; each 12-dimensional component realizes one M₁ module with
# multiplicity 12. Hence 4*(2*12) + 4*(1*12) = 144.
component_dims = sorted(isotypic_dims, reverse=True)
assert component_dims == [24] * 4 + [12] * 4
component_multiplicities = [dim // simple_dim for dim, simple_dim in zip(
    component_dims,
    [2] * 4 + [1] * 4,
)]
assert component_multiplicities == [12] * 8
print(f"  Representation multiplicities: {component_multiplicities}")
print(f"    4×M₂: dim=24 and mult=12 each; 4×M₁: dim=12 and mult=12 each.")
...
print(f"    Semisimple by the finite-dimensional unital *-algebra theorem")
print(f"    Killing degeneracy matches Z(A) numerically")
print(f"    A_EP ≅ M₂(ℂ)⁴ ⊕ M₁(ℂ)⁴")
