...
Run: python test/_exp_closure_pass.py
"""
import sys, io
...
# Spectral layers — _layers is a dict {lam: {'projector': ..., 'dim': ...}}
layers_dict = cso._layers
eigvals = sorted(layers_dict.keys(), reverse=True)
lam_labels = ['V1', 'V8/9', 'V7/9', 'V2/3', 'V5/9', 'V1/3']
layer_map = {lam: i for i, lam in enumerate(lam_labels)}
projectors = [layers_dict[lam]['projector'] for lam in eigvals]

# Helper: extract rho matrix from rho_moves values
def _get_rho(v):
    return v[1] if isinstance(v, tuple) else v

rho_matrices = [_get_rho(v) for v in cso.rho_moves.values()]

# Transport tensor (6x6)
T_ij = np.zeros((6, 6))
for i in range(6):
    for j in range(6):
        if i != j:
            ...
            for mat in rho_matrices:
                block = projectors[i] @ mat @ projectors[j]
...
def verify_rational_spectrum():
    ...
    for lam in eigvals:
        d = int(np.round(np.trace(layers_dict[lam]['projector']).real))
        e = None
