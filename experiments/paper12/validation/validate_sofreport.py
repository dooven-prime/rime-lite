...
DEFAULT_PAPER = ROOT / "papers" / "paper12" / "Paper XII.md"
APPENDIX_HEADING = "## Appendix C: Frozen SOFRS v1.0 Provenance Contract"
...
def extract_appendix_schema(path: Path) -> dict:
    ...
    if match is None:
        raise ValueError("Appendix C does not contain a fenced JSON schema")
...
def schema_drift_errors(paper_path: Path, schema: dict) -> list[str]:
    ...
    diff = list(
        difflib.unified_diff(
            embedded_text,
            canonical_text,
            fromfile="Paper XII Appendix C",
            tofile="schemas/sofrs/v1.0.schema.json",
            lineterm="",
        )
    )
    return ["Appendix C schema differs from the canonical SOFRS v1.0 schema", *diff]
...
def main() -> None:
    ...
    if drift_errors:
        ...
        for error in drift_errors:
            print(f"  {error}")
        raise SystemExit("Paper XII Appendix C schema drift detected.")
    print(f"PASS {args.paper} Appendix C matches {args.schema}")

    paths = args.paths or sorted(
        (PAPER_DIR / "archive" / "results").glob("*.sofreport")
    )
    if not paths:
