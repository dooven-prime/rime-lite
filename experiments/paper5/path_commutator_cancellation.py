...
def main() -> None:
    ...
    print(f"Signature (A0,A1,A2,Ainf): {result['sig']}")
    print()
    ...
    for i, j in gaps:
        witnesses = _length2_witnesses(R1, pairs, i, j)
        print(f"  S{i}->S{j}: D={int(D[i, j])}, witnesses={witnesses}")
    ...
    for i, j in r2_surviving:
        ...
        print(f"  S{i}->S{j}: D={int(D[i, j])}, active commutators={active}")
        assert int(D[i, j]) == 1

    assert len(Vs) == 10
    assert result["sig"] == (10, 2, 2, 76)
