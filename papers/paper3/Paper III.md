...
*This paper is Part III of the RIME trilogy. Paper I established the spectral decomposition and primitive sector structure. Paper II characterized the transport topology and M₂ Principle. The present paper develops Lie accessibility and proves the T7 Theorem.*
...
**Implications.** In the Rubik cube, compositional accessibility is strictly richer than Lie-generated accessibility: the continuous limit is structurally incomplete, omitting an entire class of cross-block morphisms that the discrete group action realizes. The S₃ negative controls support this: their canonical decompositions fail C0 (center completeness forces diagonal $K$), consistent with C0–C3 as non-trivial characterizing conditions. Formally, compositional accessibility strictly contains Lie-generated accessibility, and the T7 morphisms are the cross-block morphisms outside Lie-generated accessibility (§7).

**Core claim.** The trilogy establishes a single structural fact across three papers: **projector geometry $\not\subseteq$ Lie tangent geometry.** \cite{paper1} established that projector geometry exists (6 spectral layers, 9 primitive sectors). \cite{paper2} characterized the noncommutative curvature within it (M₂ principle, transport topology). This paper proves that compositional accessibility strictly exceeds Lie-generated accessibility (T7 morphisms). The spectral projector geometry of the representation is not contained in the Lie tangent geometry generated by $\{A_g = \log\rho(g)\}$ — a structural consequence of Schur's lemma and the isotypic decomposition (Layers A+B), verified on the Rubik's cube (228-dim); two S₃ systems (9-dim and 12-dim) serve as negative controls exhibiting no T7 morphisms.
...
| **Hybrid sector** | A primitive sector whose projector spans multiple blocks; when transport-active, it enables T7 morphisms across block boundaries (C2) | Paper II/III |
...
\cite{paper1} established the spectral ontology: the averaging operator $A = \frac{1}{|S|}\sum\rho(s)$ has a rational 6-layer spectrum, refines into 9 primitive sectors under Center$\{A, \mathrm{QT}_{\mathrm{all}}, \mathrm{HT}_{\mathrm{all}}\}$, and is organized by a refinement semilattice. \cite{paper2} established the static transport topology: $K_{\alpha\beta} = \max_g\|P_\alpha \rho(g) P_\beta\|_F > 0$ when two sectors share noncommutative support (sufficient condition; one Type II exception, S8$\leftrightarrow$S9), with S6 as the primary hub and S1 as isolated. Both papers study static objects — the spectral decomposition and the transport graph.

Paper III studies **dynamics**. Given the transport topology, what can actually reach what — and at what Lie-algebraic depth?

### The Central Question

Take two primitive sectors $\alpha$ and $\beta$ supported in disjoint blocks of the representation. Can a nonzero morphism exist from $\beta$ to $\alpha$?
...
All numerical data in this paper correspond to the 6 canonical layers ($k \in \{0,1,2,3,4,6\}$, $\lambda = 1 - k/9$) and 9 primitive sectors from the Center $\{A_{18}, \mathrm{QT}_{\mathrm{all}}, \mathrm{HT}_{\mathrm{all}}\}$ joint diagonalization. The single source of truth is the Unified Computational Supplement (CCS).
...
with 6 distinct eigenvalues and 9 primitive sectors (Center joint diagonalization). Full tables in (CCS-I §1.3–§1.4).
...
**Definition (Compositional accessibility).** Let $\{E_\alpha\}$ be the primitive sectors. A morphism $E_\beta \to E_\alpha$ exists in **compositional accessibility** if there exists a finite sequence of intermediate sectors $\gamma_1, \ldots, \gamma_{n-1}$ and group elements $g_1, \ldots, g_n \in G$ such that:
...
The canonical 18-generator system closes all three structural levels: arithmetic (rational spectrum), sector (9 primitive sectors), and transport (10 edges, 5 T7). Removing specific generators activates distinct failure modes. Four families were constructed by selective deletion:
...
Thus the continuous Lie tangent picture is structurally incomplete: finite compositional geometry contains accessibility channels invisible to the tangent algebra. Equivalently: projector geometry $\not\subseteq$ Lie tangent geometry.
...
**Canonical policy.** Unless otherwise stated, all sector decompositions in this trilogy are defined with respect to the transport-generated commutative algebra $Z = \langle A_{\mathrm{full}}, A_{\mathrm{trans}} \rangle$. Sectorizations involving auxiliary block projectors (e.g. $P_{\mathrm{nat}}$) are treated as externally refined decompositions and are not considered canonical.
...
**Mathematical lineage.** This paper belongs to the tradition of **Lie algebraic accessibility and geometric control theory** — the Chow-Brockett-Jurdjevic-Sussmann program of understanding nonlinear accessibility through Lie brackets of the accessibility algebra. The $\kappa_d$ hierarchy (§3) is a discrete-group analogue of the iterated Lie-bracket accessibility distributions; the T7 Theorem (§5) establishes that, on the block-decomposed representations studied here, discrete composition realizes morphisms that no element of the universal enveloping algebra $\mathcal{U}(\mathcal{L})$ produces — a structural mismatch that does not arise in the continuous theory. The lineage runs: nonholonomic systems and bracket-generating distributions (Chow 1939) → Lie algebraic accessibility and orbit theorems (Brockett 1973, Sussmann-Jurdjevic 1972) → geometric control on Lie groups (Jurdjevic 1997) → finite-dimensional representations and universal enveloping algebras (Serre 1977, Humphreys 1972) → the present paper.

### Lie algebraic accessibility and geometric control

[1] W.-L. Chow, "Über Systeme von linearen partiellen Differentialgleichungen erster Ordnung." *Mathematische Annalen* 117:98–105, 1939.
  — The foundational result that a bracket-generating distribution makes the orbit equal to the ambient manifold (Chow-Rashevskii theorem). The block-preserving Lemma~\ref{lem:lie-generated-support-invariance} is the opposite phenomenon for our representation: the Lie algebra $\mathcal{L} = \operatorname{Lie}\{A_g\}$ is *not* bracket-generating across blocks, so the Lie orbit is strictly smaller than the compositional reach.

[2] R.W. Brockett, "Lie Theory and Control Systems Defined on Spheres." *SIAM Journal on Applied Mathematics* 25(2):213–225, 1973.
  — Foundational paper for Lie-algebraic controllability: Lie brackets of drift and control vector fields generate the accessibility algebra. The $\kappa_d$ hierarchy is the discrete-group analogue — $\kappa_0$ from individual $A_g$, $\kappa_1$ from first brackets $[A_g, A_h]$, $\kappa_d$ from depth-$d$ commutators.

[3] H.J. Sussmann and V. Jurdjevic, "Controllability of Nonlinear Systems." *Journal of Differential Equations* 12:95–116, 1972.
  — The orbit theorem: the reachable set from a point is the immersed submanifold whose tangent space is the accessibility algebra evaluated at that point. The central T7 question — "what can the discrete composition reach beyond the Lie orbit?" — is the discrete counterpart.

[4] V. Jurdjevic, *Geometric Control Theory*. Cambridge University Press, 1997.
  — Comprehensive treatment of Lie-algebraic controllability on Lie groups: drift fields, control fields, accessibility distributions, and the Chow-Rashevskii theorem. The continuous-control framework against which T7 is measured and shown to be strictly richer.

[5] H. Nijmeijer and A.J. van der Schaft, *Nonlinear Dynamical Control Systems*. Springer, 1990.
  — Accessibility distributions and the Lie algebraic rank condition for nonlinear control systems. The iterative construction of the accessibility algebra by Lie-bracket closure is the continuous ancestor of the $\kappa_d$ depth hierarchy.

### Universal enveloping algebra and finite-group representations

[6] J.-P. Serre, *Linear Representations of Finite Groups*. Graduate Texts in Mathematics 42, Springer, 1977.
  — Schur's Lemma, isotypic decomposition, and the structure of $\mathbb{C}[G]$-modules. The block decomposition $V = V_A \oplus V_B$ and the block-preserving property of $\mathcal{L}$ (Lemma~\ref{lem:lie-generated-support-invariance}) are consequences of the isotypic structure of $\rho$. The T7 condition C1 — shared isotypic support — is the necessary intertwiner obstruction read off from this decomposition.

[7] J.E. Humphreys, *Introduction to Lie Algebras and Representation Theory*. Graduate Texts in Mathematics 9, Springer, 1972.
  — Standard reference for universal enveloping algebras $\mathcal{U}(\mathcal{L})$, the Poincaré-Birkhoff-Witt theorem, and the relation between Lie-algebra and group-algebra representations. The statement "$\overline{\mathcal{T}} \setminus \mathcal{L}$ contains morphisms unreachable by any element of $\mathcal{U}(\mathcal{L})$" (see the Structure Behind T7) is the categorical reformulation of Lemma~\ref{lem:lie-generated-support-invariance} in this language.

### Category-theoretic language

[8] S. Mac Lane, *Categories for the Working Mathematician*, 2nd ed. Graduate Texts in Mathematics 5, Springer, 1998.
  — Standard reference for the elementary categorical notions used in the Structure Behind T7 section (category, subcategory, faithful functor, compositional closure). The transport category $\overline{\mathcal{T}}$ and the Lie sublayer $\mathcal{L}$ are used as structural language to compare two generating systems; no advanced category theory is invoked.

### Trilogy cross-references

\cite{paper1}, \cite{paper2}, and \cite{ccs} are defined in `papers/tex/trilogy.bib`.
